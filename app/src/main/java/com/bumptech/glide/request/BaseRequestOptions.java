...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    public T sizeMultiplier(float f) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().sizeMultiplier(f);
        }
        if (f < 0.0f || f > 1.0f) {
    ...
    public T useUnlimitedSourceGeneratorsPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().useUnlimitedSourceGeneratorsPool(z);
        }
    ...
    public T useAnimationPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().useAnimationPool(z);
        }
    ...
    public T onlyRetrieveFromCache(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().onlyRetrieveFromCache(z);
        }
    ...
    public T placeholder(int i) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().placeholder(i);
        }
    ...
    public T fallback(int i) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().fallback(i);
        }
        this.fallbackId = i;
    ...
    public T override(int i, int i2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().override(i, i2);
        }
    ...
    public <Y> T set(Option<Y> option, Y y) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().set(option, y);
        }
    ...
    final T optionalTransform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    final T transform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    T transform(Transformation<Bitmap> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().transform(transformation, z);
        }
    ...
    <Y> T transform(Class<Y> cls, Transformation<Y> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo213clone().transform(cls, transformation, z);
        }
