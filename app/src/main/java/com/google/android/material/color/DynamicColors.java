            ...
            if (dynamicColorsOptions.getPrecondition().shouldApplyDynamicColors(activity, themeOverlay)) {
                if (dynamicColorsOptions.getContentBasedSeedColor() != null) {
                    if (MaterialAttributes.resolveBoolean(activity, R.attr.isLightTheme, true)) {
                        schemeDarkContent = Scheme.lightContent(dynamicColorsOptions.getContentBasedSeedColor().intValue());
                    } else {
                        schemeDarkContent = Scheme.darkContent(dynamicColorsOptions.getContentBasedSeedColor().intValue());
                    }
                    ColorResourcesOverride cc = ColorResourcesOverride.CC.getInstance();
                    if (cc == null || !cc.applyIfPossible(activity, MaterialColorUtilitiesHelper.createColorResourcesIdsToColorValues(schemeDarkContent))) {
                        return;
                    }
                } else {
